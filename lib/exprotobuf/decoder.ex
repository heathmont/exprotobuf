defmodule Protobuf.Decoder do
  ...
  defp convert_field(value, msg, %OneOfField{name: field}) do
    ...
    cond do
      is_tuple(inner_value) ->
        ...
        converted_value =
          {key, inner_value |> Utils.convert_from_record(module) |> convert_fields}
  ...
  defp unwrap_scalars_walker(%module{} = val, %Field{}, %{} = msg_defs) do
    module
    ...
    |> case do
      ...
      false ->
        maybe_unwrap_enum(val, msg_defs)
    end
  end

  defp unwrap_scalars_walker(val, _, %{}) do
    val
  end

  defp maybe_unwrap_enum(%module{} = val, %{} = msg_defs) do
    msg_defs
    |> Map.get(module)
    |> case do
      %MsgDef{
